...
def harrison_ford
  ...
  #Actor.joins(:movies).where("castings.ord != 1 AND name = 'Harrison Ford'")
  #Movie.joins(:actors).where("castings.ord != 1 AND name = 'Harrison Ford'")
  Movie.joins(:actors).where(actors: {name: 'Harrison Ford'}).where.not(castings: {ord:1})

  #Movie.select(:movie_id, :title)
end
